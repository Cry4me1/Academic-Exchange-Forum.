...
export async function POST(req: Request): Promise<Response> {
    ...
    // 无积分记录或余额不足时拒绝（注册奖励由数据库触发器发放，
    // 老用户补发已由 20260228 迁移处理，此处不再以用户会话铸造积分）
    if (!creditData || creditData.balance < MIN_CREDIT_COST) {
        return new Response(
            JSON.stringify({ error: "INSUFFICIENT_CREDITS", balance: creditData?.balance ?? 0 }),
