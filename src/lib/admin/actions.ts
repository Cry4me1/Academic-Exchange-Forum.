...
/**
 * 创建用于管理权限的 Supabase 客户端 (需要 SUPABASE_SERVICE_ROLE_KEY)
 */
...
export async function resetUserPassword(userId: string, newPassword: string) {
  ...
  if (error) throw new Error(`密码重置失败: ${error.message}`);
...
export async function updateUserBadges(
  ...
  if (error) throw new Error(`更新勋章/称号失败: ${error.message}`);

  await logAdminAction({
    actionType: "user_badges_updated",
    targetType: "user",
    targetId: userId,
    details: { special_title: specialTitle, badges },
  });
